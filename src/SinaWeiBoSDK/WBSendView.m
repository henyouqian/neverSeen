...
- (id)initWithAppKey:(NSString *)appKey appSecret:(NSString *)appSecret text:(NSString *)text image:(UIImage *)image
{
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, 480)])
    {
		...
		[logoutButton setFrame:CGRectMake(15, 296, 48, 30)];
		[logoutButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
		[logoutButton setBackgroundImage:[UIImage imageNamed:@"btn.png"] forState:UIControlStateNormal];
		[logoutButton setTitle:NSLocalizedString(@"登出", nil) forState:UIControlStateNormal];
...
{
    [engine logOut];
    if ([delegate respondsToSelector:@selector(sendViewLogout:)])
    {
        [delegate sendViewLogout:self];
    }
    [self hide:YES];
}
